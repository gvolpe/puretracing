...
class OpenTracingTracing[F[_]](tracer: io.opentracing.Tracer)(implicit F: Sync[F]) extends Tracer[F] {
  ...
  override def setTag(span: Span, key: String, v: TracingValue): F[Unit] =
    v match {
      case TracingValue.StringTracingValue(value) => F.delay(span.setTag(key, value)).void
      case TracingValue.NumberTracingValue(value) => F.delay(span.setTag(key, value)).void
      case TracingValue.BooleanTracingValue(value) => F.delay(span.setTag(key, value)).void
    }
  ...
  override def log(span: Span, fields: Seq[(String, TracingValue)]): F[Unit] = {
    ...
    F.delay(span.log(jFields)).void
  }

  override def setBaggageItem(span: Span, key: String, value: String): F[Unit] = F.delay(span.setBaggageItem(key, value)).void
